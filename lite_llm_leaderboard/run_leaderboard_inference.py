...
class CaptionMatcher:
    def __init__(
        self, model: str, instances_json: str, images_dir: str, 
    ...
    def prepare_instance_message(self, instance: Dict) -> List[Dict]:
        ...
        base64_image = self.encode_image(os.path.join(self.images_dir, instance["image"]))
...
Remember to provide your final answer in the format "Final Answer: X" where X is the letter of your choice.""".format(
    ...
    def process_instance(self, instance: Dict) -> Dict:
        ...
        if "instance_id" not in instance:
            raise ValueError(f"Missing required 'instance_id' field in instance: {instance}")
    ...
    def run(self) -> List[Dict]:
        ...
        print(f"Processing {len(to_process)} instances with batch API using {self.max_workers} workers...")
        batch_messages = []
        batch_ids = []
        ...
        try:
            ...
            for instance_id, instance in tqdm(to_process, desc="Individual processing"):
                try:
                    ...
                    batch_results.append(error_result)
                    print(f"Error in individual processing for {instance_id}: {individual_error}")
    ...
    def save_results(self, output_file: str = None) -> None:
        ...
        if output_file is None:
            output_file = f"predictions_{self.model.replace('/', '_')}_{int(time.time())}.json"
        ...
        print(f"Processed {len(results)} instances with {errors} errors in {duration:.2f} seconds")
        if len(results) > 0:
...
try:
    ...
    print("Unit tests not found in test_caption_matcher.py. Tests will not be available.")


def parse_args():
    parser = argparse.ArgumentParser(description="Run LLM inference for New Yorker caption matching")
    parser.add_argument("--model", required=True, help="Model identifier (e.g., gemini-1.5-pro-002)")
    parser.add_argument("--instances", required=True, help="Path to instances JSON file")
    parser.add_argument("--images_dir", required=True, help="Directory containing cartoon images")
